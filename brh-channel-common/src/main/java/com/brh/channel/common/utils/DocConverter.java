...
public class DocConverter {
	...
	public DocConverter(String fileString) {
		...
		if(os.toLowerCase().startsWith("win")){  
		  System.out.println(os + ":windows环境");  
		  environment=1;
		}else{
		  System.out.println(os + ":Linux环境"); 
		  environment=2;
		}  
	}
		...
		if (docFile.exists()) {
			if (!pdfFile.exists()) {
				...
				try {
					...
					e.printStackTrace();
					System.out.println("****pdf转换器异常，openoffice 服务未启动！****");
					throw e;
				} catch (com.artofsolving.jodconverter.openoffice.connection.OpenOfficeException e) {
					e.printStackTrace();
					System.out.println("****pdf转换器异常，读取转换文件 失败****");
					throw e;
				} catch (Exception e) {
					e.printStackTrace();
					throw e;
	...
	public boolean conver() {
		if (pdfFile.exists()) {
			System.out.println("****pdf转换器开始工作，该文件已经转换为 pdf****");
			return true;
		}
		if (environment == 1) {
			System.out.println("****pdf转换器开始工作，当前设置运行环境 windows****");
		} else {
			System.out.println("****pdf转换器开始工作，当前设置运行环境 linux****");
		}
		try {
			doc2pdf();
			//pdf2swf();
	...
	public String getpdfPath(){
		if (this.pdfFile.exists()){
			...
			tempString = tempString.replaceAll("\\\\", "/");
			System.out.println("pdf最后文件路径为"+tempString);
			return tempString;
		} else {
			return "文件不存在";
		}
