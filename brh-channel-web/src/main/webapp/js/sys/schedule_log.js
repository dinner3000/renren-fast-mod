$(function () {
    ...
    $("#jqGrid").jqGrid({
        url: '../sys/scheduleLog/scheduleInfo/' + jobId,
        datatype: "json",
        colModel: [			
            { label: '日志ID', name: 'logId', width: 60, key: true },
			...
			{ label: '状态', name: 'status', width: 50, formatter: function(value, options, row){
				return value === 0 ? 
					'<span class="label label-success pointer"  onclick="vm.showError('+row.logId+')">成功</span>' :
