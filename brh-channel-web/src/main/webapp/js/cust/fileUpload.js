...
var uploadTools = {
    ...
    "getInitOption":function(uploadId){
        ...
        var initOption={
            ...
            "canDrag":true,//是否可以拖动
            "fileType":['jpeg','bmp','png','JPG','jpg','docx','doc','ppt','pptx','pps','xls','xlsx','pdf','txt'],//"*",//文件类型
            "size":"-1",//文件大小限制,单位kB
...
//                alert("上传成功");
    ...
    "initWithCleanFile":function(opt){
        ...
        $("#"+uploadId+" .uploadBts .cleanFileBt").on("click",function(){
            ...
            $('.selectFileBt').css("background-color","#337ab7");
        });
    ...
    "initWithLayout":function(opt){
        ...
        btsStr += "<div>";
        btsStr += "<div class='selectFileBt btn btn-primary' style = 'border-color: rgb(221, 221, 221);'><i class='fa fa-plus uploadicon'></i>选择文件</div>";
        btsStr += "</div>";
        btsStr += "<div class='uploadFileBt'>";
        // btsStr += "<i class='iconfont icon-shangchuan'></i>";
        btsStr += "<a class='btn btn-primary'><i class='fa fa-level-up uploadicon'></i>上传</a>";
        btsStr += " </div>";
        btsStr += "<div class='cleanFileBt'>";
        // btsStr += "<i class='iconfont icon-qingchu'></i>";
        btsStr += "<a class='btn btn-primary'><i class='fa fa-trash-o uploadicon'></i>清除</a>";
        btsStr += " </div>";
        btsStr += "</div>";
    ...
    "addFileList":function(fileList,opt){
        ...
        for(var i=0;i<fileList.length;i++){
            ...
            var fileTypeLowercaseStr = fileTypeStr.toLowerCase();
            //文件类型判断
            if(fileTypeArray=="*"||uploadTools.isInArray(fileTypeLowercaseStr,fileTypeArray)){
//                var fileTypeUpcaseStr = fileTypeStr.toUpperCase();
                if(imgtest.test(fileList[i].type)){
                    ...
                    var fileModel = uploadTools.getShowFileType(true,fileTypeLowercaseStr,fileList[i].name,imgUrlStr,fileListArray.length);
                    $(boxJsObj).append(fileModel);
                }else{
                    var fileModel = uploadTools.getShowFileType(true,fileTypeLowercaseStr,fileList[i].name,null,fileListArray.length);
                    $(boxJsObj).append(fileModel);
                }
    ...
    "uploadFile":function(opt){
        ...
        formData.append("optOtherData",opt.otherData);   //除文件外其他参数在这里添加
...
//                		 debugger;
                		 var zTree = window.top.zTree;
                		 var treeNode = window.top.treeNode;
                		 var errorStr =""; 
                		 var fileInfo ; 
                		 var resList = data.resList;
                		 for(var i = 0; i < resList.length ; i++ ){
                			 fileInfo = resList[i].fileInfo;
                			 errorStr = resList[i].errorStr;
                			  zTree.addNodes(treeNode, 
                            	{	
                            		id:fileInfo.idNo,              //每个文件夹或文件的主键
                                    fileId:fileInfo.idNo,
                            		parentId:treeNode.tId, 
                            		fileNm:fileInfo.fileNm,
                            		fileCode:treeNode.fileCode,   //父节点的fileCode
                            		folderLvl:fileInfo.folderLvl,
                            		fileCls:fileInfo.fileCls,
                            		upFileNo:fileInfo.upFileNo,
                            		fileTyp:fileInfo.fileTyp,
                            		filePath:fileInfo.filePath,
                            		incomNo:fileInfo.incomNo,
                            		ciNo:fileInfo.ciNo
                            	}
                			  );  
                		 }
                		...
                		top.layer.close(index);
//                		var	bytesRead = 0;
//                		 uploadTools.showUploadProgress(opt, bytesRead, 100);
                		 alert("上传失败！");
                	}
                  //setTimeout(function(){opt.onUpload(opt,data)},3000);
                	uploadTools.initWithSelectFile(opt);//初始化选择文件按钮
                    uploadTools.initWithUpload(opt);//初始化上传
                    uploadTools.initWithCleanFile(opt);
//                    uploadTools.cleanFileEvent(opt);
                },
                error:function(e){
